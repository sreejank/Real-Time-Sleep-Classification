...
// initialize VT_Data
void VT_Data_Init( VT_Data* that, const PL_Event* data )
{
    that->timestamp = ( ((unsigned __int64)data->UpperTS) << 32 ) | data->TimeStamp;
...
CString VT_Acc_Print( const VT_Acc* that )
{
    ...
    switch( VT_Acc_Mode(that) ) {
        case CENTROID:
            result.Format( "CENTROID\nTimestamp: %I64u\nX: %u\nY: %u", 
                that->timestamp, that->cx, that->cy );
            break;
        case CENTROID_WITH_MOTION:
            result.Format( "CENTROID WITH MOTION\nTimestamp: %I64u\nX: %u\nY: %u\nMotion: %u", 
                that->timestamp, that->cx, that->cy, that->cm );
            break;
        case LED_1:
            result.Format( "LED #1\nTimestamp: %I64u\nX: %u\nY: %u", 
                that->timestamp, that->x1, that->y1 );
            break;
        case LED_2:
            result.Format( "LED #2\nTimestamp: %I64u\nX: %u\nY: %u", 
                that->timestamp, that->x2, that->y2 );
            break;
        case LED_3:
            result.Format( "LED #3\nTimestamp: %I64u\nX: %u\nY: %u", 
                that->timestamp, that->x3, that->y3 );
            break;
        case LED_12:
            result.Format( "LED #1 & #2\nTimestamp: %I64u\nX1: %u\nY1: %u\nX2: %u\nY2: %u", 
                that->timestamp, that->x1, that->y1, that->x2, that->y2 );
            break;
        case LED_13:
            result.Format( "LED #1 & #3\nTimestamp: %I64u\nX1: %u\nY1: %u\nX3: %u\nY3: %u", 
                that->timestamp, that->x1, that->y1, that->x3, that->y3 );
            break;
        case LED_23:
            result.Format( "LED #2 & #3\nTimestamp: %I64u\nX2: %u\nY2: %u\nX3: %u\nY3: %u", 
                that->timestamp, that->x2, that->y2, that->x3, that->y3 );
            break;
        case LED_123:
            result.Format( "LED #1 & #2 & #3\nTimestamp: %I64u\nX1: %u\nY1: %u\nX2: %u\nY2: %u\nX3: %u\nY3: %u", 
